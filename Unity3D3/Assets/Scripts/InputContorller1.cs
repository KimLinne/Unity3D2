...
public class InputContorller1 : MonoBehaviour
{   
    ...
    private void moving()
    {
        //블렌드 트리 사용 시 겟엑시스로우는 사용하지 말 것. 변화 수치 -1,0,1 3개.
    ...
    private void activeDanceInventory()
    {
        ...
        //for (int iNum = 0; 1000000 < iNum; ++iNum)
        //{
        //    Instantiate();
        //}
    }
    ...
    private void checkAim()
    {
        if (Input.GetKeyDown(KeyCode.LeftControl) && doChangeState == false)
        {
            ...
            {
                Cursor.lockState = CursorLockMode.Locked; //커서가 보이지 않아야함.
                //layer weight 를 1로.
                //anim.SetLayerWeight(1, 1); // 1번 레이어의 웨이트 값을 1로.
            ...
            else 
            {
                ...
                //layer weight 를 0으로.
                //anim.SetLayerWeight(1, 0); // 1번 레이어의 웨이트 값을 0으로.
        ...
        if (Cursor.lockState == CursorLockMode.Locked)
        {
            ...
            {
                //anim.Play("원하는 애니메이션 이름"); 총 쏘는 애니메이션 등.
            }
        }
    ...
    IEnumerator changeState(bool _upper)
    {
        ...
        if (_upper)
        {
            while (anim.GetLayerWeight(1) < 1.0f) //1번레이어가 1의 값보다 낮다면
            {
        ...
        {
            while (anim.GetLayerWeight(1) > 0f) //1번 레이어가 0의 값보다 높다면
            {
                ratio += Time.deltaTime * 5f;
...
//IEnumerator cor()
//{
//yield return new WaitForSeconds(1.0f); //다음 코루틴으로 보내고 코드가 스스로 1초를 기다림. 반복문으로 이용해선 안됨.
                                         //메모리 누수가 있을 수 있다.
            //반복문의 경우에는 아래 timer 를 사용할 것 메모리 낭비가 없다.
            //float timer = 0.0f;
...
//yield return new WaitForEndOfFrame(); // 1프레임을 넘겨준다.
